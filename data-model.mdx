...
Whether you are using Plain's API or not, this page provides a quick way to understand how Plain works.

Within Plain, all data belongs to a **Workspace**. Within a workspace, you have Users, Customers, Threads, Companies, and more.

## Workspace

Everything within Plain happens in a workspace. Typically, you will have one workspace like you would in Slack or Discord. You can create multiple workspaces if you want to match different environments, for example "Acme Staging" and "Acme Production".
...
Customers are created automatically by Plain when receiving a new support request or can be created programmatically using our API. Customers can belong to a company and multiple tenants (more on those below).
...
Labels are a lightweight way of categorizing threads by topic (e.g., bugs, feature requests, demo requests, etc.). A thread can have one or more labels, and each label has a name and an icon that you choose in your settings.

You can filter threads by label in any queue.

## Events

Events belong to a customer or thread and allow you to log important actions that happen outside of Plain. Events provide you with additional context about the customer's actions when you are helping them.
...
You can filter and organize your support by company.

## Tenants

Tenants allow you to organize your customers in the same way they are organized in your product. For example, if your product's users are organized into teams, then every team would be one tenant within Plain.

Tenants can only be created programmatically and are useful for advanced integrations or larger support teams.

## Tiers

Tiers allow you to organize companies and tenants into groups that match your product's pricing. For example "Enterprise", "Pro", "Free", etc. Within tiers, you can also define SLAs so that you can stay on top of your queue and prioritize effectively.