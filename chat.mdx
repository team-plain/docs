...
img-src https://prod-uk-services-workspac-workspacefilespublicbuck-vs4gjqpqjkh6.s3.amazonaws.com https://i0.wp.com;
```

<Note>
  The `img-src` directive URLs are required for the chat widget to display the workspace logo from a S3 bucket owned by Plain and the agents' profile images, which are powered by WordPress' Gravatar service.
</Note>