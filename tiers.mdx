...
Tiers can have a default priority. This allows you to, for example, set all Enterprise support requests to have a priority of "High". This enables you to encourage prioritization by tier (e.g., "Enterprise first, then Pro, then everyone else").
...
When a thread is created via the API for a specific tenant, the tier is selected as follows:
...
- If the tenant is not a member of any tiers, we will use the default tier if one exists, or the thread will have no tier

If a thread is created with no tenant, we will fall back to the logic below.

#### Email & Slack

For all email and Slack threads, the tier is applied based on the customer the thread belongs to. We look at all tenants and companies the customer is part of to determine the tier:

- If there is a single possible tier given the customer, that tier is used
...
SLAs can also be driven by thread priorities. This allows you to have different SLAs for "Urgent" vs. "Normal" priority threads. If you have configured multiple SLAs for the same priority, only the first SLA will apply.

If you want to create a 'default' SLA, you should first create a tier and make that the default in your settings. Then you can configure SLAs like you would on any other tier.
...
We also provide the [Thread SLA status transitioned](/api-reference/webhooks/thread-service-level-agreement-status-transitioned/) webhook so that you can be notified programmatically. This event will be fired when the status of an SLA linked to a thread changes (e.g., about to breach, breaching, achieved, etc.).
...
This is because there are 30 minutes of business hours on Friday afternoon, after which your SLA is paused over the weekend. This is followed by a remaining 30 minutes of business hours from 9:00am - 9:30am on Monday.