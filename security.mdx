...
Achieving SOC 2 compliance means that Plain has implemented procedures, policies, and controls necessary to meet AICPA's trust services criteria for security, availability, and confidentiality. These processes and controls have been tested to ensure they are operating effectively.

Obtain a copy of the report by emailing us at [help@plain.com](mailto:help@plain.com).

## Data security

- We use Amazon Web Services (AWS) to host Plain
- All data is stored in AWS `eu-west-2` (London) region
...
- We apply the following security best practices:
  ...
  - We grant the least amount of privileges to AWS Identity and Access Management (IAM) roles, systems, and engineers to perform their duties
  - Administrator privileges are only used in case of serious incidents. For routine maintenance tasks, we provision IAM roles with fine-grained permissions
- We carefully evaluate third-party vendors before using them and regularly review them and the data they can access. Please see the [Data Processing Addendum](https://www.plain.com/legal/dpa/) for the full list of vendors we use.

## Request signing

Outbound requests we make to your target URLs provide an HMAC signature with a shared secret key. Please see the [Request signing](/api-reference/request-signing) documentation for more information.

## Reporting an issue

If you think you have found a security issue or have any questions related to security, please email us at **[security@plain.com](mailto:security@plain.com)**.
